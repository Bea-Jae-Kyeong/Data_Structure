...
public:
	...
	void insert_edge(int source, int destination, string word) {
		if (isFindVertex(source) == true && isFindVertex(destination) == true) {
			...
			edge* newEdge = new edge(srcVertex, dstVertex, word);		//totaledgelist에 추가될 새로운 edge (E1)

			TotaledgeList->insert(newEdge);

			edge* tempEdge1 = new edge(srcVertex, dstVertex, word);		//src.incidentedges 에 추가될 새로운 edge (E2) 
			edge* tempEdge2 = new edge(srcVertex, dstVertex, word);		//destination.incidentedges에 추가될 새로운 edge(E3)
	...
	void DFS(vertex* cur) {
		if (cur->visited == true)
			...
			cout << cur->data << " ";
			cur->visited = true;
			edge* curedge = cur->incidentEdgeList->head;
			for (int i = 0; i <= vertexSize;
				i++) {

				if (curedge->explore == false) {
					vertex* opposite = NULL;
					curedge->explore == true;
					if (curedge->source == cur)
						opposite = curedge->destination;
					else
						opposite = curedge->source;
...
int main() {
	...
	for (int i = 0; i < M; i++) {
		...
		else g->insert_edge(temp1, temp2 ,u);
	}


	vertex* a = g->findVertex(K);
	g->DFS(a);
	system("pause");
	return 0;
}